...
const apiUrl = `http://c1670python.pythonanywhere.com/admin/pacientes`;
...
function renderUserCards(data) {
  ...
  data.data.forEach((user) => {
    ...
    userCard.querySelector(".date-incription p").textContent =
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  navSelection.addEventListener("click", (event) => {
    ...
    if (target.classList.contains("nav-selection--inactive")) {
      ...
      active.classList.replace(
        "nav-selection--active",
        "nav-selection--inactive"
      );
